...
async function handleCheckoutSessionCompleted(
  ...
  if (pentestType && validCreditTypes.includes(pentestType)) {
    ...
    try {
      ...
      console.warn(
        "Could not retrieve line_items, defaulting to quantity 1:",
        e,
      );
    }
    ...
    try {
      ...
      console.log(
        `Added ${quantity} ${pentestType} credit(s) to user ${userId}`,
      );
    } catch (error) {
      ...
      try {
        ...
        console.log(
          `Created user ${userId} with ${quantity} ${pentestType} credit(s)`,
        );
      } catch (createError) {
