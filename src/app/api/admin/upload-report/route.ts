...
import { sendPentestReportReadyEmail } from "@/lib/email/resend";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Notify the client their report is ready (fire-and-forget; never throws).
    const pentestData = pentestDoc.data();
    const ownerId: string | undefined = pentestData?.userId;
    ...
    if (ownerId) {
      const userSnap = await admin
        ...
        .doc(ownerId)
        .get();
      clientEmail = (userSnap.data()?.email as string) || null;
    }
    await sendPentestReportReadyEmail({
      userEmail: clientEmail,
      target: pentestData?.targetUrl || "your target",
      type: pentestData?.type || "",
      pentestId,
    });
